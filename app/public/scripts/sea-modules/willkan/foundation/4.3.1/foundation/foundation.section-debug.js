define("willkan/foundation/4.3.1/foundation/foundation.section-debug", [ "willkan/foundation/4.3.1/foundation/foundation-debug", "$-debug" ], function(require) {
    var Foundation = window.Foundation || require("willkan/foundation/4.3.1/foundation/foundation-debug");
        ...
        Foundation.libs.section = {
            ...
            set_active_from_hash: function() {
                ...
                sections.each(function() {
                    ...
                    regions.each(function() {
                        ...
                        if (selected) {
                            ...
                            if (data_slug && new RegExp(data_slug, "i").test(hash)) {
                                if (!region.hasClass(self.settings.active_class)) region.addClass(self.settings.active_class);
                                selected = true;
                            } else {
                                region.removeClass(self.settings.active_class);
                            }
                    ...
                    if (!selected && !settings.deep_linking && (settings.one_up || !self.is_horizontal_nav(section) && !self.is_vertical_nav(section) && !self.is_accordion(section))) regions.filter(":visible").first().addClass(self.settings.active_class);
                });
            },
            reflow: function() {
